            ...
            <div className="bg-white rounded-t-3xl border border-slate-100 px-5 pt-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={async () => { const ph = (cust.phone || '').replace(/[^0-9]/g, ''); if (!ph) { alert('Khách chưa có số điện thoại'); return; } setBusy('twilio'); try { const r = await api.twilioCall(ph); alert(r.ok ? `✅ AI đang gọi ${r.to}…` : `⚠️ ${r.error || 'Không gọi được'}`); } catch (e: any) { alert(e?.message); } finally { setBusy(''); } }} disabled={busy === 'twilio'} className="inline-flex items-center gap-1.5 px-3 py-2 bg-fuchsia-600 text-white rounded-xl font-black text-xs hover:bg-fuchsia-700 disabled:opacity-60">{busy === 'twilio' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Bot className="w-4 h-4" />} Gọi AI</button>
