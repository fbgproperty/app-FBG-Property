  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Khách hàng (Leads)" value={data.stats.totalLeads} icon={Users} trend="+12% tháng này" trendUp />
          ...
          <div className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-black text-gray-900 flex items-center gap-2">
                <Activity className="w-5 h-5 text-indigo-600" /> Biểu đồ danh sách khách hàng tiềm năng
              </h3>
              {/* UI filter (chưa nối API) */}
          ...
          <div className="bg-indigo-900 rounded-[32px] p-6 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={simulating}
                className="w-full py-3 bg-white/10 hover:bg-white/20 border border-white/5 rounded-2xl flex items-center gap-3 px-4 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              >
            ...
            {simulating && (
              <div className="mt-6 pt-6 border-t border-white/10 space-y-3">
                ...
                <div className="bg-black/20 rounded-xl p-3 font-mono text-[9px] h-24 overflow-y-auto space-y-1">
                  {simLog.map((log, i) => (
                    <p key={i} className={i === 0 ? 'text-yellow-400' : 'text-indigo-200'}>
                      {log}
                    </p>
                  ))}
                </div>
